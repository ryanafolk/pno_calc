...
for h in ./all_asc_narrowbuffer/*; do
...
join -j 1 -o 1.2,1.3,1.4,2.4 <(awk '{print $1"-"$2" "$0}' ./model_dataframe/${i}.xyz | sort -k1,1 ) <(awk '{print $1"-"$2" "$0}' ./cropped_environment/${g}_${i}.xyz | sort -k1,1 ) | sed '/-9999/d' > ./joined_data/${g}_${i}.xyz
...
cut -d" " -f4 ${f} | sort -n | sed -n '1p;$p' > tmp.tmp # First line is min second line is max
